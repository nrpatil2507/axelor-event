...
public class EventRegistrationServiceImpl implements EventRegistrationService {
  ...
  public BigDecimal setAmount(Event event, EventRegistration eventRegistration) throws Exception {
    ...
    if (CollectionUtils.isNotEmpty(discountList)) {
      for (Discount discount : discountList) {
        if (!eventRegistrationDate.isAfter(
            registrationCloseDate.minusDays(discount.getBeforeDays()))) {
  ...
  @Override
  public Boolean checkEventRegistrationDate(Event event, EventRegistration eventReg)
      throws Exception {
  ...
  public Boolean checkCapacity(Event event) {
    if (event.getTotalEntry() < event.getCapacity()) return true;
    return false;
  }
  ...
  public void setTotalAmount(Event event, EventRegistration eventRegistration) {
    ...
    event.setTotalEntry(totalEntry + 1);
  ...
  public void removeData(EventRegistration eventRegistration) {
    ...
    BigDecimal totalDiscount =
        Beans.get(EventServiceImpl.class).computeTotalDiscount(event, amountCollected);
    event.setAmountCollected(amountCollected);
    event.setTotalDiscount(totalDiscount);
    eventRepo.save(event);
  }
